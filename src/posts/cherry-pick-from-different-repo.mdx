...
desc: "다른 Git 저장소(repository)의 커밋을 현재 프로젝트로 가져오는 cherry-pick 방법과 충돌 해결 방법을 설명합니다."
date: 2025-01-07T13:17:23.170Z
...
- 명령어 : `git remote add <remote-name> <repository-url>`
...
<Callout type="info">
  2circumflex은 제 깃허브 아이디입니다. 자신의 아이디로 변경해주세요.
</Callout>
...
- 명령어 : `git log <remote-name>/<branch-name>`
...
- 명령어 : `git cherry-pick <commit-hash>`
...
- 명령어 : `git cherry-pick <commit-hash> <commit-hash> <commit-hash>`
...
이 명령어는 `<start-hash>`를 제외하고 `<end-hash>`까지 가져옵니다.

- 명령어 : `git cherry-pick <start-hash>..<end-hash>`

```shell
git cherry-pick 4ff5bf0..973dcb0
```

커밋 c(1d293ba)부터 커밋 e(973dcb0)까지 가져옵니다.

### 커밋을 범위로 가져오기 2

`<start-hash>`를 포함해서 `<end-hash>`까지 가져옵니다.
...
커밋 b(4ff5bf0)부터 커밋 e(973dcb0)까지 가져옵니다.

<Callout type="danger">start-hash가 최초 커밋이면 에러가 발생합니다.</Callout>
